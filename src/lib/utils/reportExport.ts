...
import { formatTimeFromHours } from './calibration';
...
function drawQualitySection(ctx: CanvasRenderingContext2D, result: CalibrationResult, y: number) {
  ...
  ctx.fillStyle = result.confidence === 'high' ? '#10b981' : result.confidence === 'medium' ? '#f59e0b' : '#ef4444';
  ctx.font = 'bold 16px sans-serif';
  ctx.fillText(result.confidence === 'high' ? '高' : result.confidence === 'medium' ? '中' : '低', 590, y + 70);
...
function drawComparisonSection(ctx: CanvasRenderingContext2D, result: CalibrationResult, y: number) {
  ctx.fillStyle = '#1e293b';
  ctx.beginPath();
  ...
  const items = [
    { label: '影长', theoretical: result.comparison.shadowLength.theoretical.toFixed(2), measured: result.comparison.shadowLength.measured.toFixed(2), unit: 'm', diff: result.comparison.shadowLength.difference },
    { label: '影子角度', theoretical: result.comparison.shadowAngle.theoretical.toFixed(1), measured: result.comparison.shadowAngle.measured.toFixed(1), unit: '°', diff: result.comparison.shadowAngle.difference },
    { label: '太阳高度角', theoretical: result.comparison.solarAltitude.theoretical.toFixed(1), measured: result.comparison.solarAltitude.measured.toFixed(1), unit: '°', diff: result.comparison.solarAltitude.difference },
    { label: '太阳方位角', theoretical: result.comparison.solarAzimuth.theoretical.toFixed(1), measured: result.comparison.solarAzimuth.measured.toFixed(1), unit: '°', diff: result.comparison.solarAzimuth.difference },
  ];
  ...
  items.forEach((item, i) => {
    ...
    ctx.fillStyle = Math.abs(item.diff) < 1 ? '#10b981' : Math.abs(item.diff) < 5 ? '#f59e0b' : '#ef4444';
    ctx.fillText(diffStr + ' ' + item.unit, colX[3], rowY + 5);
  });
}

function drawDeviationSection(ctx: CanvasRenderingContext2D, result: CalibrationResult, y: number) {
  ctx.fillStyle = '#1e293b';
  ctx.beginPath();
  ctx.roundRect(40, y, 720, 160, 12);
  ctx.fill();
...
function drawStepsSection(ctx: CanvasRenderingContext2D, result: CalibrationResult, y: number) {
  ctx.fillStyle = '#1e293b';
  ctx.beginPath();
  ctx.roundRect(40, y, 720, 280, 12);
  ctx.fill();
...
function drawFooter(
  ...
  ctx.font = '11px sans-serif';
  ctx.textAlign = 'left';
  ctx.fillText(`测量时间: ${new Date(input.measurementDateTime).toLocaleString('zh-CN')}`, 40, y + 10);
  ctx.fillText(`位置: ${input.locationName || `${input.latitude.toFixed(2)}°N, ${input.longitude.toFixed(2)}°E`}`, 40, y + 28);
...
<body>
  ...
  <p><strong>日晷类型：</strong>${input.sundialType === 'equatorial' ? '赤道式' : input.sundialType === 'horizontal' ? '水平式' : '垂直式'}</p>
  ...
  <h2>四、校准步骤</h2>
  ${result.calibrationSteps.map(step => `
...
export function printReport(input: CalibrationInput, result: CalibrationResult): void {
