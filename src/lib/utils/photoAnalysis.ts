...
export function estimateShadowDirectionFromAngle(angleDegrees: number, horizonAngle: number = 90): number {
...
function analyzeImageData(
  ...
  if (shadowMinX !== -1) {
    ...
    gnomonTip = { x: shadowMinX, y: Math.max(0, baseY - (shadowRow - baseY) * 0.8) };
...
export function snapToHorizon(
  ...
  const horizonY = horizon.start.y +
...
export function getAngleBetweenPoints(p1: PhotoAnalysisPoint, p2: PhotoAnalysisPoint): number {
  const dx = p2.x - p1.x;
  const dy = p2.y - p1.y;
...
export function applyAnalysisToInput(
  ...
  if (analysis.gnomonLengthPixels > 0 && analysis.shadowLengthPixels > 0) {
    shadowLength = (analysis.shadowLengthPixels / analysis.gnomonLengthPixels) * gnomonRealLength;
  }

  if (analysis.shadowTip && analysis.shadowBase) {
    const angle = getAngleBetweenPoints(analysis.shadowBase, analysis.shadowTip);
    if (analysis.horizonLine) {
      const horizonAngle = getAngleBetweenPoints(analysis.horizonLine.start, analysis.horizonLine.end);
