<script lang="ts">
  ...
  import { Calendar, Sun, Snowflake, Leaf, Flower2 } from 'lucide-svelte';
    ...
    {#each keyDateGroups as group}
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          {#each group.dates as dateItem}
            {@const dateData = getDateData(group.mode, dateItem.key, (dateItem as any).index)}
            {#if dateData}
              <button
                onclick={() => goToDate(dateData.date)}
                class="p-2 bg-slate-800/40 hover:bg-slate-700/50 rounded-lg border border-slate-700/50 
                       hover:border-amber-500/30 transition-all text-left"
              >
                <div class="text-xs font-medium text-slate-200">{dateItem.label}</div>
                <div class="text-xs text-slate-500 mt-0.5">{formatDate(dateData.date)}</div>
